...
client.on("message", message => {
 ...
 if (message.content === "^^help") {
  ...
  const embed = new Discord.RichEmbed()
      .setColor("RANDOM")
      .addField("『Havana SyStem』 『اسم البوت』", true)
...
client.on('ready', () => {
   client.user.setGame(" ^^help | Havana ");
}); 
...
if (message.content.startsWith('^^bc-users')){
 if(!message.author.id === '538337750701572106') return;
...
client.on('message', message => {
 if (message.content.startsWith("ترحيب 1")) {
  ...
  .setDescription('```شكراّ علي دخولك السيرفر```')
